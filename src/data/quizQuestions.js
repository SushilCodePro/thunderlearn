export const quizQuestionsByCourse = {
  'js-basics': [
    {
      id: 1,
      question: 'Why did Netscape want a new language in the browser in the 1990s?',
      options: [
        'To replace HTML entirely',
        'So web pages could respond instantly without waiting on the server',
        'To compete with Python on the desktop',
        'Because Java was too fast',
      ],
      correct: 1,
      explanation: 'Early pages were static. Netscape wanted the web to react instantly in the browser — that need seeded JavaScript.',
    },
    {
      id: 2,
      question: 'What was Java\'s famous promise that made it attractive for the web?',
      options: [
        '"Write once, run anywhere"',
        '"One language to rule them all"',
        '"No plugins required"',
        '"Built in ten days"',
      ],
      correct: 0,
      explanation: 'Java promised write once, run anywhere via the JVM — perfect for a web of different computers.',
    },
    {
      id: 3,
      question: 'What is the difference between "heavy lifting" and "glue" code on a web page?',
      options: [
        'Heavy lifting is CSS; glue is HTML',
        'Heavy lifting is big boxed programs; glue is small page interactions like clicks and form checks',
        'They are the same thing',
        'Glue code runs on the server only',
      ],
      correct: 1,
      explanation: 'Java handled heavy components in a box. JavaScript was the lightweight glue woven into the page.',
    },
    {
      id: 4,
      question: 'How long did Brendan Eich take to write the first JavaScript prototype?',
      options: ['Ten months', 'Ten weeks', 'Roughly ten days', 'One year'],
      correct: 2,
      explanation: 'The JavaScript prototype was written in roughly ten days in May 1995.',
    },
    {
      id: 5,
      question: 'How related are Java and JavaScript?',
      options: [
        'They are the same language with different names',
        'JavaScript is a subset of Java',
        'As related as "car" and "carpet" — mostly a marketing name',
        'JavaScript compiles to Java bytecode',
      ],
      correct: 2,
      explanation: 'JavaScript was named to borrow Java\'s fame. The two languages are largely unrelated despite similar syntax.',
    },
    {
      id: 6,
      question: 'Why could JavaScript touch the page but Java applets could not easily?',
      options: [
        'JavaScript was woven into the document; Java ran in a sealed box',
        'Java was faster so browsers blocked it',
        'JavaScript had more memory',
        'Java did not support functions',
      ],
      correct: 0,
      explanation: 'JavaScript lived in the page itself. Java applets ran isolated and could not easily interact with page elements.',
    },
    {
      id: 7,
      question: 'Why did browsers eventually remove Java applets (~2015–2017)?',
      options: [
        'Java was too easy for beginners',
        'Security holes, slow startup, and JavaScript engines caught up in speed',
        'Sun Microsystems shut down',
        'HTML5 replaced all programming',
      ],
      correct: 1,
      explanation: 'Java needed plugins, had security issues, and booted slowly. Fast JS engines and native graphics erased Java\'s advantages.',
    },
    {
      id: 8,
      question: 'What role does JavaScript play next to WebAssembly today?',
      options: [
        'WebAssembly replaces JavaScript entirely',
        'JavaScript is still the glue — WebAssembly cannot touch the page directly',
        'They cannot work together',
        'WebAssembly handles all UI events',
      ],
      correct: 1,
      explanation: 'WebAssembly is the new heavyweight. To change buttons or react to clicks, it must call out to JavaScript — still the glue.',
    },
    {
      id: 9,
      question: 'Why can\'t browsers run arbitrary C++ from websites safely?',
      options: [
        'C++ is too slow',
        'C++ could access files, run system commands, and overwrite memory without a full sandbox rewrite',
        'C++ does not support strings',
        'Browsers only support one language',
      ],
      correct: 1,
      explanation: 'Raw C++ has unrestricted file, memory, and system access — a massive security nightmare in a browser tab.',
    },
    {
      id: 10,
      question: 'Typical home PC RAM in 1995 was about:',
      options: ['16 GB', '4–8 MB', '512 MB', '1 GB'],
      correct: 1,
      explanation: 'Average PCs had 4–8 MB RAM. A heavy C++ runtime was impossible alongside Windows 95 and the browser.',
    },
    {
      id: 11,
      question: 'What is the main difference between let and const?',
      options: [
        'let is only for strings',
        'const can be reassigned but let cannot',
        'let can be reassigned; const cannot be reassigned',
        'They are exactly the same',
      ],
      correct: 2,
      explanation: 'let allows reassignment. const keeps the variable binding fixed, though object properties can still change.',
    },
    {
      id: 12,
      question: 'Which list contains only primitive JavaScript data types?',
      options: [
        'object, array, function',
        'number, string, boolean, undefined, null, bigint, symbol',
        'div, button, input',
        'map, set, weakmap',
      ],
      correct: 1,
      explanation: 'The primitive types covered in Lecture 02 are number, string, boolean, undefined, null, bigint, and symbol.',
    },
    {
      id: 13,
      question: 'What happens when obj2 = obj1 and then obj2.name is changed?',
      options: [
        'Only obj2 changes because objects copy by value',
        'obj1 can also show the change because both variables reference the same object',
        'JavaScript throws an error',
        'Both objects become primitive values',
      ],
      correct: 1,
      explanation: 'Objects are non-primitive values. Assigning obj2 = obj1 copies the reference, not a fresh object.',
    },
    {
      id: 14,
      question: 'What does the % operator return?',
      options: [
        'Percentage',
        'Power',
        'Remainder after division',
        'Strict equality',
      ],
      correct: 2,
      explanation: 'The modulo operator % returns the remainder. For example, 10 % 4 returns 2.',
    },
    {
      id: 15,
      question: 'Why is === usually preferred over ==?',
      options: [
        '=== ignores data type',
        '=== checks type and value without loose conversion surprises',
        '=== only works with strings',
        '=== converts both sides to numbers first',
      ],
      correct: 1,
      explanation: 'Strict equality checks both type and value. Loose equality can convert values before comparing.',
    },
    {
      id: 16,
      question: 'Why should money often be stored as paise/cents instead of decimal rupees/dollars?',
      options: [
        'Integers avoid floating-point precision problems',
        'Strings are always faster',
        'Math.random requires it',
        'Browsers cannot display decimals',
      ],
      correct: 0,
      explanation: 'Floating-point math can produce precision surprises like 0.1 + 0.2. Storing smallest units as integers avoids that for finance.',
    },
    {
      id: 17,
      question: 'Which loop is guaranteed to run at least once?',
      options: ['for loop', 'while loop', 'do-while loop', 'forEach only'],
      correct: 2,
      explanation: 'A do-while loop runs the body first, then checks the condition.',
    },
    {
      id: 18,
      question: 'Which formula generates an integer from min to max, inclusive?',
      options: [
        'Math.random(min, max)',
        'Math.floor(Math.random() * (max - min + 1) + min)',
        'Math.ceil(max - min)',
        'Number.random(max)',
      ],
      correct: 1,
      explanation: 'The Lecture 04 range formula is Math.floor(Math.random() * (max - min + 1) + min).',
    },
    {
      id: 19,
      question: 'Which string method removes spaces from the beginning and end?',
      options: ['split()', 'slice()', 'trim()', 'replaceAll()'],
      correct: 2,
      explanation: 'trim() returns a new string with leading and trailing whitespace removed.',
    },
    {
      id: 20,
      question: 'Why were data types invented?',
      options: [
        'To make code look longer',
        'Because the same bytes can mean different things without type context',
        'To stop browsers from loading CSS',
        'Because JavaScript cannot store text',
      ],
      correct: 1,
      explanation: 'Memory is just bytes. A type tells the computer how to interpret those bytes and what operations are valid.',
    },
    {
      id: 21,
      question: 'Which variable keyword is function-scoped and discouraged in modern JavaScript?',
      options: ['const', 'let', 'var', 'symbol'],
      correct: 2,
      explanation: 'var is function-scoped, can be redeclared, and is hoisted as undefined. Modern code usually prefers const and let.',
    },
    {
      id: 22,
      question: 'What does typeof null return in JavaScript?',
      options: ['null', 'undefined', 'object', 'boolean'],
      correct: 2,
      explanation: 'typeof null returns "object", a long-standing JavaScript bug learners should recognize.',
    },
    {
      id: 23,
      question: 'Why does 2 + 3 * 5 evaluate to 17?',
      options: [
        'JavaScript reads all operators from right to left',
        'Multiplication has higher precedence than addition',
        'Addition always happens last',
        'The expression is invalid',
      ],
      correct: 1,
      explanation: 'Operator precedence runs multiplication before addition. Use parentheses when you want a different order.',
    },
    {
      id: 24,
      question: 'Which values are falsy in JavaScript?',
      options: [
        'false, 0, "", null, undefined, NaN',
        '"false", [], {}',
        'Every object',
        'Every string',
      ],
      correct: 0,
      explanation: 'The common falsy values are false, 0, empty string, null, undefined, and NaN. Empty arrays and objects are truthy.',
    },
    {
      id: 25,
      question: 'What is the safest explanation for 0.1 + 0.2 not exactly equaling 0.3?',
      options: [
        'JavaScript uses random math',
        'Decimal fractions can become repeating binary floating-point values',
        'The + operator is broken',
        '0.3 is not a number',
      ],
      correct: 1,
      explanation: 'JavaScript numbers use binary floating point, so some base-10 decimals are stored as approximations.',
    },
    {
      id: 26,
      question: 'Which Number constant helps compare floating-point results that are close enough?',
      options: ['Number.MAX_VALUE', 'Number.MIN_VALUE', 'Number.EPSILON', 'Number.NaN_VALUE'],
      correct: 2,
      explanation: 'Number.EPSILON is useful when checking whether two floating-point results are close enough.',
    },
    {
      id: 27,
      question: 'What is special about NaN?',
      options: [
        'It is equal to itself',
        'It is the only value in JavaScript that is not equal to itself',
        'typeof NaN is string',
        'It only appears in arrays',
      ],
      correct: 1,
      explanation: 'NaN means Not a Number, but typeof NaN is "number"; NaN === NaN is false.',
    },
    {
      id: 28,
      question: 'What is the difference between slice(-2) and substring(-2) on a string?',
      options: [
        'They always return the same result',
        'slice can count from the end with negative indexes; substring treats negative values like 0',
        'substring works only on arrays',
        'slice deletes the original string',
      ],
      correct: 1,
      explanation: 'slice supports negative indexes. substring does not use negative indexes the same way.',
    },
  ],
  'dom-events': [
    {
      id: 1,
      question: 'Which method selects the first element matching a CSS selector?',
      options: ['getElementById', 'querySelector', 'getElementsByTag', 'findElement'],
      correct: 1,
      explanation: 'document.querySelector(".class") returns the first matching element in the DOM.',
    },
    {
      id: 2,
      question: 'How do you attach a click handler to a button?',
      options: [
        'button.onclick = handleClick',
        'button.addEventListener("click", handleClick)',
        'button.on("click", handleClick)',
        'Both A and B are valid',
      ],
      correct: 3,
      explanation: 'Both onclick assignment and addEventListener work. addEventListener is preferred — it allows multiple handlers.',
    },
  ],
  'async-js': [
    {
      id: 1,
      question: 'What does await do inside an async function?',
      options: [
        'Pauses the entire browser',
        'Waits for a Promise to resolve before continuing',
        'Creates a new thread',
        'Converts sync code to sync code',
      ],
      correct: 1,
      explanation: 'await pauses the async function until the Promise settles, then returns the resolved value.',
    },
  ],
}

export const getQuizQuestions = (courseId) =>
  quizQuestionsByCourse[courseId] ?? []
